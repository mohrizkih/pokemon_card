...
class CardsViewmodel extends ChangeNotifier {
  ...
  int get currentData => hasCards ? (_state as CardsLoaded).cardResponse.data.length : (isLoading ? ((_state as CardsLoading).cardResponse?.data.length ?? 0) : 0);
  ...
  Iterable<PokemonCard> get cardsRelated => isRelatedCardLoaded ? (_stateDetail as DetailCardLoaded).cardResponse.data : [];
  ...
  void onTapCard(PokemonCard card, {bool isFromDetail = false}) {
    ...
    if (_pokemonCard != null) {
      fetchRelated(type: _pokemonCard?.types.firstOrNull?.type ?? '', supertype: _pokemonCard?.supertype.type ?? '');
